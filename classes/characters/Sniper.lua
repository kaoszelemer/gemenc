local Sniper = Character:extend('Sniper')
...
function Sniper:init(x, y)
    ...
    "sniper",
    love.graphics.newImage("assets/sniper.png"),
    30,
    30,
    20,
    1,
...
function Sniper:update(dt)
    ...
    if LEVEL == GLOBALS.bossonwhichlevel then
       -- print(distance)
        ...
        if distance < 95 and distance > 93 and not self.isDead then
        ...
        if distance < 95 and not self.isDead and not self.EnemyBulletshot then
            self:action(x, y)
        end
    end
...
function Sniper:action(x,y)

    

    if self.EnemyBulletshot ~= true then
        self.drawMarkerline = true
        Timer.after(3, function ()
            self.drawMarkerline = false
        end)
